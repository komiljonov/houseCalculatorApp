...
class Bot:
    def __init__(self, token: str):
        ...
        self.app.add_handler(
            ConversationHandler(
                ...
                {
                    MENU: [
                        MessageHandler(filters.Text(["Kalkulator"]), self.calculator),
                        MessageHandler(filters.Text(["Savol berish"]), self.question)
                    ],
                    HOME_PRICE: [
                    ...
                    # ],
                    QUESTION: [
                        MessageHandler(filters.TEXT & EXCLUDE, self.question_text)
                    ]
                },
                [
                    CommandHandler('start',self.start)
        ...
        self.app.add_handler(
            MessageHandler(filters.ChatType.GROUP & filters.REPLY & filters.TEXT & EXCLUDE,self.group_question_answer)
        )
        self.app.add_handler(
            MessageHandler(filters.ChatType.PRIVATE & filters.REPLY & filters.TEXT & EXCLUDE,self.group_question_answer_user)
        )
    ...
    async def group_question_answer(self,update:Update,context:CallbackContext):
        ...
        question = Question.objects.filter(group_message_id=update.message.reply_to_message.message_id).first()
    ...
    async def group_question_answer_user(self,update:Update,context:CallbackContext):
        ...
        question = Question.objects.filter(user_message_id=update.message.reply_to_message.message_id).first()
